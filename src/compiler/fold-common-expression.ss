(library (compiler fold-common-expression)
  ...
  (define-pass fold-common-expression : L14 (x) -> L14 ()
	...
	'(['< . <]
	  ...
	  [>= . >=]))
      (define env '()))
    (Effect : Effect (compiler ir) -> Effect ()
	    [(set! ,x ,[v])
	     (begin
	       (if (or (int64? v) (label? v) (uvar? v))
		   (set! env (cons (cons x v) env)))
	       ir)])
    (Value : Value (compiler ir) -> Value ()
	   [,triv (cond
    ...
    (Tail : Tail (compiler ir) -> Value ()
	  ...
	  [(,binop ,[v1] ,[v2])
	   (if (and (int64? v1) (int64? v2))
	       ((cdr (assq binop binop-map) v1 v2))
	       `(binop ,v1 ,v2))])
    (Pred : Pred (compiler ir) -> Pred ()
	  [(,relop ,[v1] ,[v2])
	   (if (and (int64? v1) (int64? v2))
	       (if ((cdr (assq relop relop-map)) v1 v2) `(true) `(false))
	       `(relop ,v1 ,v2))])
    (Body : Body (compiler ir) -> Body ()
	  [(locals (,local* ...) ,[tbody])
	   (begin
	     (set! env '())
	     `(locals (,local* ...) ,tbody))])
